...
//            String query = "select CASE WHEN actor_id > 100 THEN 'High' ELSE 'Low' END from actor where actor_id > 100 and actor_id < 150";
//            String query = "SELECT actor_id, CASE WHEN actor_id < 100 THEN 'Low' WHEN actor_id < 200 THEN 'Medium' ELSE 'High' END as actor_id_category FROM actor";
//            String query = "select -(actor_id+actor_id)*actor_id as agg from actor where actor_id > 100 and actor_id < 150 order by (actor_id+actor_id)*actor_id";
//            String query = "select first_name from actor where actor_id > 100 and actor_id < 150 and first_name like 'A_am'";
//            String query = "select first_name from actor where first_name like 'Spencer*'";
//            String query = "select first_name, actor_id from actor where actor_id < 5";
//            String query = "select first_name, last_name, avg(actor_id) from actor GROUP BY Grouping Sets ((last_name, first_name), (last_name), (first_name), ()) having count(*) > 1";
//            String query = "select max(last_name), COUNT(DISTINCT last_update), sum(actor_id) from actor GROUP BY last_name, first_name";
//            String query = "select count(Distinct first_name) from actor";
            ...
            // sort the result
            result.sort(Comparator.comparing(a -> ((String) a[0])));
    ...
    public void testJoinAggregateSort_1() {
        try{
            ...
            String query = "select film_id, count(film_id) as cnt\n"
                ...
                + "group by film_id order by cnt DESC, film_id ASC\n";

//            String query = "select max(customer_id) from\n"
//                    + "(select p.customer_id, count(distinct p.amount) as tot_amount\n"
//                    + "from rental r right join payment p\n"
//                    + "on r.rental_id = p.rental_id and r.customer_id = p.customer_id and 2 = r.staff_id and p.staff_id = 1\n"
//                    + "group by p.customer_id order by tot_amount DESC, p.customer_id ASC\n)"
//                    + "as foo group by tot_amount\n"
//                    + "order by tot_amount";
            ...
            for(int key : expected.keySet()){
                ...
                for(int i = 0; i < expected.get(key).size(); i++){
                    System.out.println("RES: " + expected.get(key).get(i) + " == " + actual.get(key).get(i));
    ...
    public void testSort(){
        try{
            ...
            String query = "select f.title from\n"
                + "(select title, length, rental_rate from film where rating = 'PG')\n"
                + "as f where f.length < 50 order by f.title DESC\n";
//                + "as f where f.length < 50 order by f.length ASC, f.title DESC";
//                + "OFFSET 1 ROWS\n"
//                + "FETCH NEXT 5 ROWS ONLY";
    ...
    public void testBonus(){
        try{
            MyCalciteConnection calciteConnection = new MyCalciteConnection();
            String query = "select first_name, last_update from actor order by first_name, last_name, last_update";
